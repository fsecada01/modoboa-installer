...
from .. import database, package, system, utils
...
class Dovecot(base.Installer):
    ...
    def get_config_files(self):
        ...
        return self.config_files + [
            "dovecot-sql-{}.conf.ext=dovecot-sql.conf.ext".format(
                self.dbengine,
            ),
            "dovecot-sql-master-{}.conf.ext=dovecot-sql-master.conf.ext".format(
                self.dbengine,
            ),
            "postlogin-{}.sh=/usr/local/bin/postlogin.sh".format(self.dbengine),
        ]
    ...
    def get_packages(self):
        ...
        if backports_codename:
            packages = [
                f"{package}/{backports_codename}-backports"
    ...
    def get_template_context(self):
        ...
        un, uid, gid, dir_str = get_user_info(username=self.mailboxes_owner)
        ...
        context.update(
            {
                ...
                "mailboxes_owner_uid": uid,
                "mailboxes_owner_gid": gid,
    ...
    def restore(self):
        ...
        if len(os.listdir(mail_dir)) > 0:
            ...
            utils.printcolor(
                "It seems that emails were not backed up, skipping "
                "restoration.",
                utils.MAGENTA,
            )
